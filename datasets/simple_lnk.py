...
def compute_lnk_response_from_convolved(x_center: torch.Tensor,
                                      x_surround: torch.Tensor,
                                      lnk_params: Dict[str, np.ndarray],
                                      device: torch.device,
                                      dtype: torch.dtype,
                                      rgc_noise_std: float = 0.0) -> torch.Tensor:
    """
    Compute LNK response from pre-convolved center and surround signals.
    
    This function takes already convolved x_center and x_surround signals and applies
    the LNK adaptation dynamics and output nonlinearity.
    
    LNK Model (from convolved signals):
        1. Adaptation: a_{t+1} = a_t + dt * (alpha_d * max(0, x_c - theta) - a_t) / tau
        2. Output: r_t = softplus(g_out * (x_c + w_xs * x_s) / (sigma0 + alpha * a_t) + beta * a_t + b_out)
    
    Args:
        x_center: Pre-convolved center signals [N, T]
        x_surround: Pre-convolved surround signals [N, T]
        lnk_params: Dict with LNK parameters
        device: Device for computation
        dtype: Data type for computation
        rgc_noise_std: Standard deviation of noise to add to combined input
    ...
    # Ensure inputs have the same time length
    T_out = min(x_center.shape[1], x_surround.shape[1])
    x_center = x_center[:, :T_out] * _data_scaling_factor_4_fitting
    x_surround = x_surround[:, :T_out] * _data_scaling_factor_4_fitting
    ...
    if _is_log_distribution_stats:
        ...
        logging.info("=" * 60)
        logging.info("LNK Response Distribution Analysis (from convolved)")
        logging.info("=" * 60)
        log_distribution_stats(x_center, "x_center")
        log_distribution_stats(x_surround, "x_surround") 
        log_distribution_stats(combined_input, "combined_input")
